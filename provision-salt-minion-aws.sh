...
is_service_loaded() {
  if which systemctl > /dev/null 2> /dev/null; then
    ...
    if status $1 2> /dev/null | grep -qi "^$1"; then
      echo "1"
...
is_service_active() {
  if which systemctl > /dev/null 2> /dev/null; then
    ...
    if status $1 2> /dev/null | grep -qi "^$1 start/running,"; then
      echo "1"
...
is_package_installed() {
  if which apt > /dev/null 2> /dev/null; then
    if apt list $1 --installed 2> /dev/null | grep -qi "^$1\/"; then echo "1"; else echo "0"; fi
  else
    if yum list installed $1 2> /dev/null | grep -qi "^$1\."; then echo "1"; else echo "0"; fi
  fi
}
...
if which salt-call > /dev/null 2> /dev/null; then
